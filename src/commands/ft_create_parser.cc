...
absl::Status ParseTextIndexOptions(vmsdk::ArgsIterator &itr,
  ...
  index_schema_proto.set_text_language(data_model::LANGUAGE_ENGLISH);
  ...
  while (itr.HasNext()) {
    ...
    if (absl::EqualsIgnoreCase(current_param, kNoStemParam)) {
      ...
      index_schema_proto.set_text_language(data_model::LANGUAGE_UNSPECIFIED);
      itr.Next();
      continue;
    }
    ...
    if (absl::EqualsIgnoreCase(current_param, kLanguageParam)) {
      ...
      if (lang_it != kLanguageByStr->end()) {
        index_schema_proto.set_text_language(lang_it->second);
        index_schema_proto.set_nostem(false);
      } else {
...
absl::StatusOr<data_model::IndexSchema> ParseFTCreateArgs(
  ...
  if (!HasVectorIndex(index_schema_proto) && !HasTextIndex(index_schema_proto)) {
    return absl::InvalidArgumentError(
